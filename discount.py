    ...
    def on_change_discount_formula(self):
        if self.discount_formula is not None and self.base_price is not None:
            self.unit_price = self.apply_discount_formula(
                self.base_price, self.discount_formula, raise_exception=False)
...
class ApplyDiscountMixin(Model):

    @classmethod
    def apply_discount_formula(cls, unit_price, formula, raise_exception=True):
        def is_number(value):
            try:
                float(value)
                return True
